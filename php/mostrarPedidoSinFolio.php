...
$sd =new conexion();
$sd->conectar();
$pedidosx = mssql_query("select s.nombre sala,s.localizacion,folio_solicitudidfk,observaciones,sr.estatusidfk,t.nombre tecnico,su.nombre supervisor
from tsolicitud_refaccion sr inner join csala s on (s.salaid=sr.salaidfk) inner join ctecnico t on (t.tecnicoid=sr.tecnicoidfk)
inner join csupervisor su on (su.supervisorid=t.supervisoridfk) where solicitud_refaccionid=".$sol." ");
...
$tecx=mssql_query("select tecnicoidfk from tsolicitud_refaccion where solicitud_refaccionid=".$sol." ");
...
<td>Tecnico: <span class='gris'>".$row["tecnico"]."</span></td><td>Supervisor: <span class='gris'>".$row["supervisor"]."</span></td></tr>
...
<img src='imgs/editarPequeno.bmp' title='Editar Version' id='camVerPed".$sol.$con."' onclick='imgAgregarCantPed(".$sol.", ".$con.", ".$piezas["refaccionidfk"].", \"".$piezas["serie"]."\", \"".$piezas["version"]."\", 2);' style='cursor: pointer;' />
</td>
...
<td   id='tdCamEstatusPed".$sol.$con."' align='center';>".$piezas["estatus"]."
</td>
<td>".$piezas["motivo"]."</td>";
echo"</tr>";}echo"</table>";
...
<div id='spanMod".$sol."' style='float:left'><input type='button' size='9' id='modificarPed' value='Modificar Sol. Refaccion' onclick='ModificarSolRefPro(".$sol.");'></div>
