...
$selectRegionAT=$_POST["selectRegionATx"];
$selectSalaAT=$_POST["selectSalaATx"];
...
if($txtSerieAT==''){
$sql = mssql_query("select cmaq.serie, tmov.fecha, tmov.origen, tmov.destino, cu.nombre nombreUsuario, tmov.origenID, tmov.destinoID, cs.nombre nombreSala
from tmovimientoMaquinas tmov
inner join cmaquinas cmaq on (cmaq.maquinaid=tmov.maquinaidfk)
inner join cusuario cu on (cu.usuarioid=tmov.usuarioidfk)
inner join csala cs on (cs.salaid=".$selectSalaAT.")
where (tmov.origen like 'sala' and tmov.origenID like '".$selectSalaAT."' or tmov.destino like 'sala' and tmov.destinoID like '".$selectSalaAT."') and (convert(char(10),tmov.fecha,103) BETWEEN convert(datetime,'".$txtFHRCalendario111."',103) AND convert(datetime,'".$txtFHRCalendario222."',103)) ");
}

/*if($selectUbicacionAT=='bodega'){
$sql = mssql_query("select cmaq.serie, tmov.fecha, tmov.origen, tmov.destino, cu.nombre
from tmovimientoMaquinas tmov
inner join cmaquinas cmaq on (cmaq.maquinaid=tmov.maquinaidfk)
inner join cusuario cu on (cu.usuarioid=tmov.usuarioidfk)
where (tmov.origen like 'bodega 102' or tmov.destino like 'bodega 102') and (convert(char(10),tmov.fecha,103) BETWEEN convert(datetime,'".$txtFHRCalendario111."',103) AND convert(datetime,'".$txtFHRCalendario222."',103))
");	
}*/
